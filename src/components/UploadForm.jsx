...
export default function UploadForm({ onSubmit, isLoading }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5 rounded-2xl border border-ink/10 bg-white p-4 shadow-lg md:rounded-3xl md:p-6">
      ...
      <label className="group relative flex cursor-pointer flex-col items-center justify-center overflow-hidden rounded-3xl border-2 border-dashed border-alert bg-alert/10 px-4 py-10 text-center shadow-lg shadow-alert/10 transition hover:bg-alert/15 md:py-12">
        <span className="absolute right-4 top-4 rounded-full bg-white px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-alert">Start here</span>
        ...
        <span className="mt-2 max-w-full truncate text-sm text-ink/70">
      ...
      {showReview && selectedFile && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/45 px-3 py-3 sm:items-center sm:justify-center">
          <div className="max-h-[92vh] w-full overflow-auto rounded-3xl bg-white p-4 shadow-2xl sm:max-w-2xl sm:p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={() => setShowReview(false)} className="rounded-full border border-ink/15 px-3 py-1 text-sm font-semibold text-ink">
                Close
              </button>
            </div>
            ...
            <div className="mt-5 rounded-2xl bg-ink px-4 py-4 text-paper">
              ...
              <p className="mt-1 truncate text-lg font-semibold">{selectedFile.name}</p>
